...
class ProjectShow {	
	public function ProjectShowComponents() {
		...
		$project_id = (int)mysqli_real_escape_string($con, $_GET["proj_id"]);
		...
		while($showDetails = mysqli_fetch_array($sql_exec)) {
			...
			$ShowQuant = mysqli_Query($con, "SELECT projects_data_quantity FROM projects_data WHERE projects_data_component_id = '$comp_id' AND projects_data_project_id = '$project_id'");
