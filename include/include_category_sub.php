...
class NameSub {
	...
	public function Sub() {
		...
		$SubCategoryName = "SELECT * FROM category_sub WHERE id BETWEEN ".$subcatfrom." AND ".$subcatto." order by id ASC";
		$sql_exec_subcatname = mysqli_Query($con, $SubCategoryName);
		...
		while ($ShowDetailsSubCatname = mysqli_fetch_array($sql_exec_subcatname)) {
			...
			$sql_exec_component_catname = mysqli_query($con, "SELECT category FROM data WHERE owner = $owner"); // Get the category ID from all components.
